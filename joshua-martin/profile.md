...
- **Most recent batch** (Pena washed, La Isabela natural, Paraiso thermal shock, Monteblanco co-ferment, all roasted ~March 29): **two-regime drift across Day 9–33**. Pre-Day-27 segment runs ~0.026/day; post-Day-27 segment accelerates to ~0.038/day (the cleanest cross-coffee multi-day acceleration this batch has shown — confirmed by all four coffees scoring 4 at conservative +1-step settings on Day 32, then Monteblanco + Pena both hitting Score 5 at +2 steps (6.125) on Day 33 with Paraiso's correction confirming the same SS-center). Single-line fit across all 34 Score-5 anchors gives grind = 0.0307·day + 5.082 (residuals widen to ±0.10 in the late-life segment, vs. ±0.06 pre-Day-27, reflecting the regime shift the linear fit doesn't capture). For prediction past Day 27, **use 0.038/day, not the pooled rate**.

```mermaid
xychart-beta
    title "Hydrangea current batch — Score-5 anchors vs day age (Z brewer)"
    x-axis "Day age" 8 --> 33
    y-axis "Grind setting" 5.30 --> 6.15
    line [5.328, 5.358, 5.389, 5.420, 5.451, 5.481, 5.512, 5.543, 5.574, 5.604, 5.635, 5.666, 5.696, 5.727, 5.758, 5.789, 5.819, 5.850, 5.881, 5.912, 5.942, 5.973, 6.004, 6.035, 6.065, 6.096]
```

_Line shows pooled OLS fit (grind = 0.0307·day + 5.082) across Day 8–33. Score-5 anchors (n=34): D9 5.45, D10 5.45, D11 5.475, D13 5.50, D14 5.50, D15 5.525, D16 5.55, D17 5.575×2, D18 5.60×4, D19 5.625, D19 5.65×2, D20 5.675×2, D24 5.825, D25 5.85, D26 5.875×3, D27 5.90×3, D28 5.95, D29 5.975, D30 6.00, D30 6.025, D31 6.05×2, D33 6.125×2. The pooled fit splits the difference between two regimes: pre-Day-27 ~0.026/day, post-Day-27 ~0.038/day._
...
| Hydrangea (most recent batch)  | ~0.026 (Day 9–27), accelerates to ~0.038 (Day 27+) | Pena, La Isabela, Monteblanco, Paraiso. **Two-regime drift** confirmed by Day 32–33 falsification: all four coffees scored 4 at the conservative +1-step setting from Day 31's SS (6.05); Day 33 Monteblanco + Pena both S5 at 6.125 (+0.0375/day from Day 31). Pooled linear fit across 34 Score-5 anchors: grind = 0.0307·day + 5.082; the pooled rate splits the difference between regimes and underpredicts late-life by ~+0.030. All four coffees share the trajectory; per-coffee differences are vocabulary-at-the-edges only (see §13). **Pena late-life note (Day 27–33)**: S4-string from Day 27→32 was initially read as walkback-finer divergence then as "bean aged out"; Day 33 S5 at 6.125 falsified both — Pena tracks the batch trend with a narrower window. **Use 0.038/day for predictions past Day 27** and bracket with +2-step (0.050) jumps when conservative +1-step settings fail. See §7. |
...
- **Hydrangea late-life acceleration regime (Day 27+)**: The pre-Day-27 fit ran ~0.026/day with residuals inside ±0.06. Past Day 27, residuals consistently fall +0.025–0.040 above the pre-Day-27 trend, with the cleanest falsification at Day 32–33: all four coffees scored 4 at the conservative +1-step setting from Day 31's SS, and Day 33 SS jumped to 6.125 (Monteblanco + Pena S5; Paraiso correction confirmed). Empirical post-Day-27 slope: **~0.038/day**. This is the cleanest multi-day cross-coffee acceleration ever recorded in this batch — promote to a distinct regime, not "wiggles around the trend." Bracket with +2-step (0.050) jumps when conservative +1-step settings fail. Matches the late-life acceleration patterns in H&S batch 1 (past Day 50) and H&S batch 3 (past Day 58).
...
- Hydrangea current batch: **two-regime drift confirmed**. Pre-Day-27 ~0.026/day; post-Day-27 ~0.038/day. The cleanest multi-day cross-coffee falsification of the single-trend assumption: Day 32 all-S4 at +1-step settings + Day 33 sibling SS jump to 6.125 (+0.0375/day from Day 31). Pena's S4-string Day 27→32 was tracking the accelerated trend, not aging out — confirmed by Day 33 S5 at 6.125 with no mixed-edge signals. **For prediction past Day 27, use 0.038/day and bracket with +2-step jumps when +1-step fails.** Matches H&S batch 1 (past Day 50) and H&S batch 3 (past Day 58) acceleration patterns.
