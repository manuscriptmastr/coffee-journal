...
- **Detecting a water shift**: if 3+ same-day cups across **different batches** (different roast dates, different ages, different varieties) all read in the **same direction** (uniformly under, or uniformly over) relative to their per-coffee predictions, the cause is almost certainly an intercept shift — water mineralization change, a recipe change, brewer-side change — not coordinated per-coffee slope errors. Per-coffee slope errors are uncorrelated across batches; intercept shifts are uniformly directional. Estimate the magnitude by computing the implied-SS shift for each cup (cup_setting − apparent_SS) and averaging: a tight cluster (e.g., −0.025 to −0.05 across three cups) means a uniform shift; a wide cluster (e.g., −0.025 to −0.10) means uneven shift impact and the magnitude needs more data to pin down.
...
| **Fine-edge SS — vivid variant**   | brilliant / vivid / "definite and unmistakeable" flavor expression + S5-quality acidity (lychee glow, raspberry brilliance, immediate clarity) + slight localized scratch / "barely tight" | scratch present but minor, doesn't dominate cup; clean enough that flavors lead          | self-call typically "sweet spot or +1 coarser"; +1 coarser cleans mouth but **risks dulling the brilliant acidity** that defines the cup. When flavors are S5-grade, holding is often correct even with the slight rasp. |
| **Fine edge of SS — muted variant** | "barely smaller volume", "slight localized roughness", "focused / introverted", "black tea forward", "pleasant perfuminess, edges softening"                                      | clean but with hints of either roughness or thickness                                    | +1 click coarser next brew (drift-tracking); flavor brilliance is suppressed, so coarser usually unlocks rather than dulls |
| **On center**                      | "loose", "open", "voluminous", "honest flavors", "musical / shimmering / melded", **acidity immediate from first sip** and integrated; cooldown grows brighter and clearer       | clean, slippery saliva, no friction or coating                                           | hold; expect drift to require +1 next day                                                |
...
| **Mild under**                     | "loose + bright + softening + slightly diffuse", acidity present but thin / far-off, "searching for tasting notes", "flimsy / one-dimensional / simple-minded tartness", "no structure" | loose, clean, possibly slightly watery; **may include "puckery" mouth without scratch** | +1 click finer                                                                           |
| **Deep under (puckery / sour variant)** | "almost no flavor — neither acidity nor sweetness", "very sharp", "drying in the puckery way", "parched", "barely there finish", "small but sharp citric like yuzu", "far-off flavors" | dry / parched / puckery (NOT scratchy, NOT clenched); pucker is the salivary-clamp sour reflex, distinct from over's surface friction | +2 clicks finer; pucker is the load-bearing under marker — distinguishes genuine under from over's scratchy/clenched discomfort |
| **Deep under (hurricane variant)** | "large + foggy + structureless", "no sweetness", "hollow / watery / little substance", flavors present in surroundings but not in cup                                             | thin, watery, no body, **no mouth discomfort**                                            | +2 clicks finer; rare past D14 in most batches                                           |
...
- **Acidity-timing displacement is an over-direction signal.** SS cups deliver acidity **immediately on first sip**. When the cup reads "reserved in first sip, grows and shifts," "felt slightly more in second half of sip than first," or "could be even more immediate," that back-loading is a mild over signature even when the acidity is brilliant when it does arrive. Pair this with mouthfeel: immediate-acidity + clean-mouth = SS; back-loaded-acidity + slight-rasp = fine-edge over (vivid variant).
- **The S5-flavors-with-S4-mouthfeel tradeoff.** When flavor expression is S5-grade (brilliant, vivid, definitive notes) but mouthfeel has localized rasp, +1 coarser is the textbook correction — but it costs acidity brilliance about as often as it saves the cup. The brilliant-acidity feature usually lives at the fine edge of SS; one click coarser dulls the very feature that made the cup memorable. **Holding the slight rasp is sometimes the right move when the flavors are at S5.** This trade is the reason same-day self-calls on these cups read "Sweet spot or +1 coarser" rather than committing one direction. Don't reflexively coarsen brilliant-acidity cups.
- **First-cup mouthfeel can be partly bean-state, not extraction-state.** Pre-rest cups (before the roaster's published rest floor) read scratchy/tight regardless of grind; over-marker descriptors at pre-rest age may be 50/50 mix of mild over plus bean-state rasp. Don't update setting from a single pre-rest cup the way you would from a fully-rested cup. Wait for at least one cup past the rest floor before treating mouthfeel signals as load-bearing.
- **Variety-template-aware diagnosis.** "Small flavor volume" / "not vivid" / "not brilliant" against the wrong variety template is a form of reference-anchor bias. A mejorado fully dialed-in tastes delicate, sugary, subtly complex with tea/cocoa/eucalyptus notes — not brilliant or vivid in the chiroso/aji/Kenyan sense. Diagnose against the coffee's own template, not the same-roaster sibling's most-vivid expression. Less-vivid varieties have the same S5 ceiling as vivid ones; the cup just *reads* differently.
- **Clenched mouthfeel = OVER, puckery mouthfeel = UNDER.** Both involve mouth-tightness/discomfort, but they are distinct sensations with opposite direction signals in this journal. Discriminator is the **flavor cluster**, not the mouthfeel intensity:
  - **Clenched / leathery / rough / rubbing**: pairs with "monochromatic / melded / flat / compressed" flavors that are PRESENT but lacking articulation. Acidity is "loud but flat," sweetness is melded-not-defined. Direction: **OVER, fix coarser.**
  - **Puckery / drying / parched**: pairs with "flimsy / one-dimensional / simple-minded / barely-there / far-off / no structure" flavors that are SMALL or ABSENT. Acidity may be "sharp / yuzu-like / drying" without sweetness anchoring it. Direction: **UNDER, fix finer.**
  - When in doubt, the question to ask is: are flavors **present-but-flat** (over) or **small/absent** (under)? Mouth-discomfort intensity alone does not call direction.
...
flowchart TD
    ...
    B -->|Scratchy/rough/powdery<br/>+ 'small' / dull flavors| C[Spiral / mild–deep over<br/>→ COARSER 1-2 clicks]
    B -->|Slight scratch / 'barely tight'<br/>+ brilliant / vivid acidity<br/>+ S5-grade flavor expression| V[Fine-edge SS — vivid variant<br/>→ HOLD or +1 coarser<br/>coarser risks dulling brilliance]
    B -->|Clenched / leathery / rough<br/>+ monochromatic / melded / flat flavors<br/>flavors PRESENT but compressed| Z[Mild–deep over<br/>→ COARSER 1-2 clicks]
    B -->|Puckery / drying / parched<br/>+ flimsy / one-dimensional / barely-there flavors<br/>flavors SMALL or ABSENT| P[Genuine under<br/>→ FINER 1-2 clicks]
    ...
    D -->|Reserved first sip /<br/>acidity grows in second half /<br/>'could be more immediate'| N[Acidity-timing-displaced<br/>fine-edge over<br/>→ HOLD or +1 coarser]
```
...
- **Self-calls must come from the maintainer, not the agent.** When the maintainer's note doesn't include an explicit setting suggestion (e.g., `**6.025?**`), do not insert one in the entry body or treat your own recommendation as a same-day SS estimate. Suggesting a self-call and then citing it as confirming evidence for direction is circular reasoning that contaminates the calibration data. If the cup descriptors don't permit a clean direction call without a self-call, say so explicitly rather than fabricating one.
- **Single-interval slopes are noisy in both directions; multi-anchor patterns are load-bearing.** A two-anchor slope estimate (one interval) can swing ±50% from the bean's underlying rate due to per-cup noise, brewer variance, or one anchor sitting slightly off-SS. Treating a single noisy interval as evidence of acceleration or deceleration produces predictions that systematically over- or undershoot. Rule of thumb: if 3+ anchors converge within ±0.005/day, trust the average; if a recent interval disagrees by more than that, treat it as noise unless mechanistic corroboration goes beyond "the bean has X documented behavior."
- **Chronic per-coffee anchor-lag requires additive correction.** When a coffee has been read mild-over for 4+ consecutive cups with self-calls consistently pulling coarser (or 4+ mild-under with self-calls pulling finer), bake in a +0.025 anchor-lag adjustment on top of the drift calculation. The pattern is the bean wanting coarser/finer than the freshest anchor's same-day SS estimate suggests, and it accumulates if not corrected. This is distinct from drift — it's a per-coffee bias that drift math alone won't catch.
- **Stratify confidence by signal alignment.** A high-confidence prediction needs: (1) multi-anchor own-slope stability, (2) same-batch sibling-slope corroboration (cross-batch siblings are softer prior, not corroboration), (3) a maintainer-authored self-call as the freshest anchor, (4) no chronic anchor-lag pattern requiring bias correction, (5) water, recipe, brewer, dose stable since the anchor cups. When all five align, predictions land within ±0.025 of true SS. When any one is missing or noisy, expect ±0.025–0.05 miss; flag the prediction as low-confidence rather than treating it like the all-aligned case.
- **Coarser-side default bias is regime-conditional, not universal.** The §9 "62% coarser corrections at S4+" pattern reflects drift-tracking on over-leaning beans; it does NOT apply when the coffee or session is in an under-leaning regime. Apply coarser-side default when recent cups for the coffee have read mild over or fine-edge SS vivid variant; apply finer-side default when recent cups have read puckery/sour, mild under, or hurricane. When the regime is mixed or unknown, default to the centered estimate without bias adjustment. **The asymmetric-loss direction flips with the regime — applying coarser-side default to an under-leaning regime compounds the miss.**
- **Audit stacked corrections before assuming they cancel.** When a prediction has multiple correction terms (anchor-lag, water shift, sibling-offset, drift acceleration) that point in opposite directions and approximately net to zero, do not commit to the prediction without separately evaluating each correction's evidential basis. A weak +correction and a weak −correction "cancelling" hides the fact that both might be wrong. If one or both rests on contested interpretation, the net prediction inherits that uncertainty; flag the prediction as low-confidence and consider a bracket-probe instead of a single committed call.
