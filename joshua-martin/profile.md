...
- **Day 23 triangulation**: All three coffees sampled confirm a shared SS window of **5.775–5.80** (center ~5.785–5.79). Monteblanco at 5.75 (same setting as Day 22) dropped to Score 3 "missing bright and refreshing, less complex" → wants **5.775+ (coarser)**. Paraiso at 5.775 Score 4 fine-edge SS "funky peach gummy, could use touch more brightness." Pena at 5.80 Score 4 coarse-edge SS "sharp, citric, mouthfilling, lacks focus, needs sweetness" → wants **5.775 (finer)**. **Revised Day 22 center upward to ~5.74–5.75** (the Day 22 "~5.725 shared center" read was biased by Monteblanco's in-the-moment "5.725" call, which Joshua then reversed to "5.775+" 24 hours later at the same setting). **Day 22→23 drift ≈ +0.038/day.** Late-life drift is accelerating smoothly, not stalling.
- **Direction correction on the Day 22 Monteblanco read**: the "soft/limp/missing bright" signature is actually **fine-edge SS** (one click fine of center, wants coarser), not coarse-side under. "Fantastic clarity" + preserved flavor cleanliness rules out deep under; the brightness is being muted by slight over-extraction without spiral signals. Joshua's Day 22 call of 5.725 (finer) was wrong; his Day 23 reversal to 5.775 (coarser) is correct. Monteblanco Day 22 at 5.75 was already 0.01–0.015 fine of center; drift accumulation over 24h deepened it to 0.03+ fine of center on Day 23, triggering the Score-3 drop without a direction change.
- **Per-coffee SS-edge fingerprints now confirmed**:
  - **Pena coarse-edge SS**: _sharp / citric / mouthfilling but lacks focus / needs sweetness to define_ (Day 23 @ 5.80). Prior mild-over signature (Day 21 @ 5.7) was _nothing shining / acidity not genuine / cocoa off / heavy tea-like_ — completely different vocabulary from the coarse-edge.
  - **Monteblanco fine-edge SS**: _soft / limp / missing bright / fantastic clarity / less complex / juicy in cooldown_ (Day 22–23 @ 5.75). Reads like under at first glance but is fine-side of SS.
  - **La Isabela / Paraiso coarse-edge SS with headroom**: _loose / funky / bright / brilliant / still-structured_, optional "small volume" (Day 22 both coffees). Varietal acidity absorbs mild coarse overshoot.
    See §13 "Coffee-specific SS-edge fingerprints."
- **Score-rate collapse as drift-acceleration tell**: Days 8–20 = 70% Score-5 rate, zero Score 3s. Days 21–23 = **zero Score 5s, three Score 3s** across 11 entries. No vocabulary shift toward extraction spirals — flavors still clean, just off-center. Signature of drift outrunning 0.025-grinder-step resolution, not of the batch falling apart. See guide "Score-rate collapse as a drift-regime signal."
...
| Hydrangea (most recent batch)  | ~0.010 → 0.038 | **Three-regime curve.** Days 8–14 (CO₂ phase): ~0.010/day. Days 14–20 (mid-life ramp): ~0.025–0.030/day. **Days 20–23 (late-life): ~0.038/day and climbing** — Score 5 rate dropped from ~70% to 0% across this transition without vocabulary shift, the signature of drift outrunning grinder resolution. All four coffees still share a center within each regime; they differ in _coarse-tolerance_ (La Isabela + Paraiso stay bright + loose past SS; Pena + Monteblanco go sharp/citric and soft/limp respectively). See §7, §13. |
