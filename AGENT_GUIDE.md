...
0. **Always look up the current date programmatically — every time, no exceptions.** Run `date` (or your environment's equivalent shell call) at the start of every prediction. Do **not** rely on the "Today's date" line in your system prompt / environment block (it can be stale or initialized wrong), your own date arithmetic from a remembered "yesterday," the date implied by the most recent journal entry, or the date you used earlier in the same multi-turn conversation (sessions can cross midnight). After getting the current date, compute Day N as `(today − roast_date)` in calendar days using the journal's counting convention (see "Day X" definition below). The roast date is the source of truth — sibling-entry Day labels are only a cross-check. If the most recent sibling entries dated today disagree with the calendar math, **the calendar wins and the new entry uses the calendar-derived Day N** even when it doesn't match the siblings sitting just above your insertion point (those siblings are mislabeled, carried forward from a prior off-by-one session). Same calendar day always shares the same Day N across all batch-mates of the same roast date. Do **not** auto-increment Day X from the previous entry of the same coffee — gaps of multiple days are common.
...
**Detecting an unflagged water shift retrospectively**: If 3+ same-day cups across **different batches** (different roast dates, different ages, different varieties) all read in the **same direction** (uniformly under, or uniformly over) relative to their per-coffee predictions, the cause is almost certainly an intercept shift — water mineralization change, recipe change, brewer change, dose change — not coordinated per-coffee slope errors. Per-coffee slope errors are uncorrelated across batches; intercept shifts are uniformly directional. The discriminator is the **cross-batch uniformity** of the deviation, not its magnitude. Estimate the shift magnitude by averaging the implied SS shift (cup_setting − apparent_SS) across the same-day cluster, and apply the shift to remaining predictions until the new baseline is dialed in. A water shift can present without any change in the maintainer's stated setup if (e.g.) a refill changed mineral composition, the maintainer forgot to flag a water-batch change, or a brewer/recipe change happened off-camera.
...
- **Confounded cups must be discounted twice — once as anchors, once as theory-seeds.** When the maintainer flags a cup as a likely dud, brewer-side defect (cold-start, mis-pour, wrong water, mis-dose), or otherwise atypical, that cup's setting and direction read are unreliable for the **next-day prediction** *and* unreliable as evidence for **batch-level theories** (per-coffee decoupling, slope shifts, narrowed windows). The repeated failure mode is: 1–2 flagged cups in a row appear to show a coffee diverging from its siblings; the agent builds a working "this coffee runs coarser/finer/slower than the rest of the batch" model; the next clean cup falsifies the divergence. Rule of thumb: a working hypothesis about a coffee's behavior **needs at least one un-flagged S4+ confirmation** before it becomes operational. Flagged cups can corroborate a clean read; they cannot start one.
