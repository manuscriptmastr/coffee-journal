...
- **Misreading the maintainer's in-cup setting self-call as a next-day anchor.** When the maintainer writes a setting suggestion in their entry body (e.g., `**5.975?**`, `**6 or 6.025?**`, `**maybe 5.95**`), that is the setting they would have brewed **today, on the same day, instead of the one they actually brewed** — i.e., it is the maintainer's same-day SS-center estimate, not a recommendation for the next day's brew. When using that self-call as an anchor for tomorrow's prediction, you must still add the daily drift (and any anchor-lag correction the profile specifies) on top of it. Treating the self-call as already-accounting-for-drift will compound a fine-side bias across consecutive predictions.
- **Suggesting self-calls in the entry body, then citing them as evidence.** Self-calls are *load-bearing inputs* to the methodology — they're the maintainer's same-day SS estimate, used to anchor next-day predictions, slope estimates, and direction calls. If the agent inserts a setting suggestion (e.g., `**6.15-6.175**`) into the entry body and then later cites it as "the maintainer's self-call confirms direction," that is circular reasoning: the agent's analysis is laundering through the journal as if it were maintainer-authored evidence. Do not write self-call notation into the entry body unless the maintainer explicitly provides it. If the cup descriptors don't permit a clean direction call without a self-call, say so explicitly in chat rather than fabricating one. Recommending a next-day setting in chat is fine; injecting same-day SS estimates into the journal is not.
